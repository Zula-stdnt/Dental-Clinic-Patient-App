...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT phone_number, email, phone_otp, otp_expires_at FROM patients WHERE patient_id = ?");
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $time_left = strtotime($row['otp_expires_at'] ?? '2000-01-01') - time();
        if ($time_left > 60) {
            $response = [
                "status" => "success", 
            ...
            if (!USE_LIVE_APIS) {
                $response["mock_otp"] = $row['phone_otp'];
            }
        ...
        if (USE_LIVE_APIS) {
            ...
            curl_setopt($ch_sms, CURLOPT_POSTFIELDS, json_encode(["receivers" => [$phone], "smsBody" => "Security Alert: Your Verification OTP is $otp_code. Valid for 5 minutes."]));
            ...
            curl_setopt($ch_email, CURLOPT_POSTFIELDS, json_encode([
                ...
                "to" => [["email" => $email]],
                "subject" => "Security Alert: Account Modification",
                "htmlContent" => "<h3>Security Update Request</h3><p>Your Verification OTP to authorize changes to your account is: <strong>$otp_code</strong></p><p>If you did not request this, please contact the clinic immediately.</p>"
            ]));
        ...
        $response = [
            "status" => "success", 
            "message" => USE_LIVE_APIS ? "Security code sent to your phone and email." : "DEV MODE: Code saved to database.",
            "masked_phone" => substr($phone, 0, 6) . '*****' . substr($phone, -2)
        ];
        if (!USE_LIVE_APIS) {
